...
pub fn QrCode(
    ...
    view! {
        <img src=move || {
            ...
            let svg_data = qr.render::<svg::Color>()
        ...
        width=move || width.get().map(|w| w.get()).unwrap_or(256.) / aspect_divisor.get().map(|a| a.get()).unwrap_or(1.)
        height=move || height.get().map(|h| h.get()).unwrap_or(256.) / aspect_divisor.get().map(|a| a.get()).unwrap_or(1.)
        />
    }
}
