...
use crate::{
    ...
    models::{Profile, RssEntry, RssFeed},
    THEME_STR,
};
...
pub fn RSSPage(logged_in: ReadSignal<bool>, user: ReadSignal<Option<Profile>>) -> impl IntoView {
    ...
    let copy_to_clipboard = move |_| {
        let _ = window()
            .navigator()
            .clipboard()
            .write_text(format!("{}/rss", "TODO").as_str());
        set_copied(true);
        start(0);
    };
    ...
    view! {
        ...
        <div class="py-12 px-0 mx-auto lg:w-[96rem]">
            ...
            <div class="flex gap-4 my-4">
                ...
                >
                    {move || {
                        if copied.get() { "Copied!".into() } else { format!("{}/rss", "TODO") }
                    }}
                    <svg
            ...
            <Show
                when=move || view_feed.get() && rss_feed.get().is_some()
                fallback=move || {
                    view! {
                        <div class="markdown">
                            <div inner_html=move || {
                                rss_content.get().unwrap_or("Loading . . .".into())
                            }></div>
                        </div>
                    }
                }
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    <h1 class="text-2xl font-bold text-nf-dark sm:text-3xl">
                                        {move || { rss_feed.get().unwrap().title }}
                                    </h1>
                    ...
                    <ul class="list-none space-y-4">
                        <For
                            ...
                            children=move |entry: RssEntry| {
                                let date = DateTime::parse_from_rfc2822(entry.pub_date.as_str())
                                    .unwrap_or_default();
                                view! {
                                    <li>
